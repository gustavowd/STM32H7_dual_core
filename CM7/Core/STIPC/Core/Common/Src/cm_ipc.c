...
#endif
#endif
//static osMessageQueueId_t  STIPCEvent = 0;
QueueHandle_t STIPCEvent = NULL;
...
void HAL_HSEM_FreeCallback(uint32_t SemMask)
{
  ...
  else if((SemMask &  __HAL_HSEM_SEMID_TO_MASK(HSEM_SYNC_ID))!= 0)
  {
    ...
    /*
    if (STIPCEvent && osKernelGetState())
    {
    	uint16_t message = STIPC_SYNC_EVENT;
    	xQueueSendToBackFromISR(STIPCEvent, &message, &yield);
...
int ipc_init(void)
{
    ...
    //osMessageQDef(STIPC_QUEUE, 10, );
    //STIPCEvent = osMessageQueueNew (10, sizeof(uint16_t), NULL);
	//STIPCEvent = xQueueCreate(10, sizeof(uint16_t));
    ...
    {
    	uint16_t message = 0;
    	BaseType_t ret = xQueueReceive( STIPCEvent, &message, portMAX_DELAY );
		if( ret == pdTRUE )
		{
